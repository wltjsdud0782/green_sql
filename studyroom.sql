...
RENAME TABLE STUDYROOM_RESERVATION TO STUDYROOM_ENTER; -- 이름변경 할지말지
...
DELETE FROM MEMBER_COUPON
   WHERE OWN_COUPON_CODE = 3;

SELECT * FROM MEMBER_COUPON WHERE MEMBER_CODE = 6;
SELECT
	OWN_COUPON_CODE
	, COUPON_CODE
...
CREATE TABLE studyroom_message (
	...
	, MEMBER_CODE INT REFERENCES studyroom_member (MEMBER_CODE)
	, TO_FROM VARCHAR(10) NOT NULL DEFAULT 'TO'
);
ALTER TABLE STUDYROOM_MESSAGE ADD COLUMN TO_FROM VARCHAR(10) NOT NULL AFTER MEMBER_CODE;
SELECT * FROM studyroom_message WHERE MEMBER_CODE = 5;
DROP TABLE studyroom_message;
